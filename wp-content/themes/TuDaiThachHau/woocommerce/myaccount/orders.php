					...
					?>
						<?php if ($is_order_number) : ?>
							...
							<?php elseif ('order-actions' === $column_id) : ?>
								...
								// Thêm nút Hủy đơn hàng nếu trạng thái đơn hàng là 'pending' hoặc 'on-hold'
								...
								if (! empty($actions)) {
									foreach ($actions as $key => $action) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										...
										echo '<a href="' . esc_url($action['url']) . '" class="woocommerce-button button ' . sanitize_html_class($key) . '" aria-label="' . esc_attr(sprintf(__('View order number %s', 'woocommerce'), $order->get_order_number())) . '">' . esc_html($action['name']) . '</a>';
									}
								}
								?>
							<?php endif; ?>
