...
<p class="woocommerce-result-count" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'role="alert" aria-relevant="all" data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === intval( $total ) ) {
		...
		printf( _n( 'Có tất cả %1$d sản phẩm', 'Có tất cả %1$d sản phẩm', $total, 'woocommerce' ) . $orderedby_placeholder, $total, esc_html( $orderedby ) );
	} else {
		...
		printf( _nx( 'Có %1$d&ndash;%2$d của %3$d sản phẩm', 'Có %1$d&ndash;%2$d của %3$d sản phẩm', $total, 'với kết quả đầu và cuối', 'woocommerce' ) . $orderedby_placeholder, $first, $last, $total, esc_html( $orderedby ) );
	}
